...
def add_staff_member(request):
    ...
    if not request.user.is_superuser:
        messages.error(request, 'Sorry only store owners can add staff members')
...
def edit_staff_member(request, staff_member_id):
    ...
    if not request.user.is_superuser:
        messages.error(request, 'Sorry only store owners can edit staff members')
        return redirect('login')
...
def delete_staff_member(request, staff_member_id):
    ...
    if not request.user.is_superuser:
        messages.error(request, 'Sorry only store owners can delete staff members')
